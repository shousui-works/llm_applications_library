...
class TestIntegrationWithGenerators:
    ...
    def test_openai_generator_uses_responses_for_web_search(self):
        ...
        with patch("openai.OpenAI") as mock_openai:
            ...
            assert called_kwargs["model"] == generator.model
            # Ensure system and user messages are forwarded into input payload
            assert called_kwargs["input"][0]["role"] == "system"
            assert called_kwargs["input"][1]["role"] == "user"
